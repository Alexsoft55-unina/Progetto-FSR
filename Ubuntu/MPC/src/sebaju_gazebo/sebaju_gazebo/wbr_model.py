...
Micromachines 2022, 13, 747, with every parameter of its Table 1 computed from the SeBaJu URDF.
...
Run `ros2 run sebaju_gazebo wbr_model` to print the parameter table.
"""
...
from sebaju_gazebo.kinematics import RobotKinematics, rpy_to_matrix
...
def load_urdf_from_package():
    share = subprocess.check_output(['ros2', 'pkg', 'prefix', 'sebaju_gazebo']).decode().strip()
    return subprocess.check_output(['xacro', f'{share}/share/sebaju_gazebo/urdf/sebaju.urdf.xacro']).decode()


def main():
    model = WBRModel(load_urdf_from_package())
    p = model.p
    print('Table 1 - SeBaJu')
