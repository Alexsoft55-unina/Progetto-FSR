"""
RoTino balance + vertical jump controller for ROS 2 Humble / Gazebo Fortress.

Port of RoTino_Ctrl_BalJumpAchieve.py (MuJoCo, position_servo leg mode):
...
# The damping term alone is clamped. Running this PD at 500 Hz instead of every physics step lets a
# joint pick up ~17 rad/s per cycle while saturated, and kv*qd then commands 840 Nm and flips sign
# every sample: a bang-bang limit cycle that threw the robot within 0.5 s of release. Capping only
# the damping keeps full proportional authority for holding the pose while bounding the energy a
# velocity spike can inject. Clamping the total torque instead also works but holds the posture worse.
LEG_DAMPING_MAX = 8.0        # Nm
...
class BalanceJumpController(Node):
    ...
    def _publish_legs(self, targets, q, qd):
        ...
        tau = [clamp(clamp(kp * (target - q[name]), -LEG_TORQUE_MAX, LEG_TORQUE_MAX)
                     + clamp(-kv * qd[name], -LEG_DAMPING_MAX, LEG_DAMPING_MAX),
