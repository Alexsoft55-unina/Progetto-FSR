...
def read_csv(path):
    ...
    for key in ('time_s', 'theta_deg', 'x_m', 'xdot_ms', 'wheel_u', 'com_z_m',
                ...
                'theta_err_deg', 's_err_m', 'push_force_N'):
        col = []
        for r in rows:
            try:
...
def metrics(d):
    ...
    # Disturbance recovery energy: integral of tau^2 dt over the disturbance response
    dist_energy = float('nan')
    if fin and taus:
        i_peak = max(range(len(fin)), key=lambda i: abs(fin[i][1]))
        t_peak = fin[i_peak][0]
        t_end_dist = t_peak + (settle if not math.isnan(settle) and settle > 0 else 3.0)
        dt = 0.002
        e_samples = [tau ** 2 for ti, tau in zip(t, taus)
                     if not math.isnan(ti) and t_peak <= ti <= t_end_dist]
        if e_samples:
            dist_energy = sum(e_samples) * dt

    # Pitch tracking error RMS over entire run
...
ROWS = [
    ...
    ('pitch_err_rms_deg', 'errore beccheggio RMS [deg]', '{:.3f}'),
    ('dist_energy_J', 'energia recupero (int tau^2 dt) [J]', '{:.2f}'),
