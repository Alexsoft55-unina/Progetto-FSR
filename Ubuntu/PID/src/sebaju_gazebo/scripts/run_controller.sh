#!/usr/bin/env bash
# Run balance_jump_controller by hand against an already-running Gazebo instance
...
#
# Example:
#   run_controller.sh -p jump_enable:=false -p planar_enable:=true
...
exec ros2 run sebaju_gazebo balance_jump_controller --ros-args --params-file "$PARAMS_FILE" "$@"
