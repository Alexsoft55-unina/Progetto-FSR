...
import sys  # Import sys for standard I/O and flush.
import threading  # Import threading for terminal input listener.
...
# =========================================================
# SLOW PI VELOCITY CONTROLLER CONFIGURATION
# =========================================================
VELOCITY_PI_ENABLE = True             # True: enable slow PI velocity control at runtime
V_REF_MAX = 0.35                      # Max forward / backward target speed (m/s)
VEL_KP = 0.35                         # Proportional gain: velocity error -> pitch lean (rad per m/s)
VEL_KI = 0.040                        # Integral gain: eliminates steady-state velocity error (rad per m/s*s)
VEL_I_MAX = 0.08                      # Anti-windup cap on velocity integral
PITCH_CORR_MAX = math.radians(4.5)   # Max allowed pitch tilt offset from equilibrium
MAX_PITCH_SLEW = math.radians(2.5)    # Slew rate limit on pitch target (rad/s) for smooth transitions
CROUCH_TRANSITION_SPEED = 0.65        # Full height transition in ~1.5 seconds
...
# ============================================================================================================
# RUNTIME INTERACTIVE VELOCITY, HEIGHT & JUMP COMMAND HANDLER
# ============================================================================================================
runtime_v_ref = 0.0
manual_jump_requested = False
runtime_crouch_target = 0.0   # 0.0 = 100% height (standard stand), 1.0 = max crouch

def apply_command(action):
    """Unified handler that updates simulation setpoints from either Viewer or Terminal."""
    global runtime_v_ref, manual_jump_requested, runtime_crouch_target
    if action == "SPEED_UP":
        runtime_v_ref = float(np.clip(runtime_v_ref + 0.10, -V_REF_MAX, V_REF_MAX))
        print(f"\n>>> [CMD] Target Velocity increased -> {runtime_v_ref:+.2f} m/s", flush=True)
    elif action == "SPEED_DOWN":
        runtime_v_ref = float(np.clip(runtime_v_ref - 0.10, -V_REF_MAX, V_REF_MAX))
        print(f"\n>>> [CMD] Target Velocity decreased -> {runtime_v_ref:+.2f} m/s", flush=True)
    elif action == "STOP":
        runtime_v_ref = 0.0
        print("\n>>> [CMD] STOP COMMANDED: Target Velocity = 0.00 m/s", flush=True)
    elif action == "RAISE":
        runtime_crouch_target = float(np.clip(runtime_crouch_target - 0.25, 0.0, 1.0))
        height_pct = int((1.0 - runtime_crouch_target) * 100)
        print(f"\n>>> [CMD] Height -> {height_pct}% (Raising / Standing taller)", flush=True)
    elif action == "LOWER":
        runtime_crouch_target = float(np.clip(runtime_crouch_target + 0.25, 0.0, 1.0))
        height_pct = int((1.0 - runtime_crouch_target) * 100)
        print(f"\n>>> [CMD] Height -> {height_pct}% (Lowering / Squatting)", flush=True)
    elif action == "JUMP":
        manual_jump_requested = True
        print("\n>>> [CMD] JUMP REQUESTED (will execute when robot is balanced)", flush=True)

def key_callback(keycode):
    """GLFW Viewer key callback (handles keys when 3D window has focus)."""
    # GLFW key codes: UP=265, DOWN=264, SPACE=32, 'W'=87, 'S'=83, 'R'=82, 'F'=70, 'J'=74
    if keycode in (265, ord('W'), ord('w')):
        apply_command("SPEED_UP")
    elif keycode in (264, ord('S'), ord('s')):
        apply_command("SPEED_DOWN")
...
def start_terminal_listener(stop_event):
    """Background listener for terminal input so typing directly in the terminal works too."""
...
def run():  # Main function that loads the robot and runs the feedback loop.
    ...
    # VELOCITY PI CONTROLLER STATE (SLOW DYNAMICS)
    vel_integral = 0.0
    ...
    term_stop_event = threading.Event()
    start_terminal_listener(term_stop_event)

    if VELOCITY_PI_ENABLE:
        print("\n" + "=" * 80)
        print("  SLOW PI VELOCITY LOOP & DYNAMIC HEIGHT CONTROL ACTIVE")
        print("  Controls work in BOTH the 3D Viewer window AND in this Terminal:")
        print("    [W] or [UP]    : Accelerate forward (+0.10 m/s, max 0.35 m/s)")
        print("    [S] or [DOWN]  : Decelerate / reverse (-0.10 m/s)")
        print("    [SPACE]        : Stop immediately (0.00 m/s)")
        print("    [R]            : Raise height (stand taller, +25%)")
        print("    [F]            : Lower height (crouch / squat, -25%)")
        print("    [J]            : Trigger jump on demand (when balanced)")
        print("=" * 80)

    print(f"  SLOW_FACTOR  = {SLOW_FACTOR:.2f}  (edit this to change simulation speed)")  # Print simulation speed setting.
    print("-" * 115)
    print(f"{'Time':>6} {'Vcmd':>6} {'Hgt':>5} {'Pitch':>8} {'CoMLean':>8} {'Rate':>7} {'Vel':>7} {'WheelU':>7} {'X':>6} {'MoveDir':>9} {'Status':>10}")
    print("-" * 115)


    with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:  # Open MuJoCo passive viewer with key callback.
        ...
        while viewer.is_running():  # Keep running until the viewer window is closed.
            ...
            # ========================================================================================
            # JUMP STAGE-AWARE BALANCER WITH SLOW PI VELOCITY LOOP
            # ========================================================================================
            if VELOCITY_PI_ENABLE and jump_state == STATE_BALANCE:
                v_ref = runtime_v_ref
                vel_err = v_ref - xdot
                vel_integral = clamp(vel_integral + vel_err * dt, -VEL_I_MAX, VEL_I_MAX)

                # Bleed integral when commanding stop and speed is nearly zero
                if abs(v_ref) < 0.01 and abs(xdot) < 0.03:
                    vel_integral *= 0.98

                # Outer PI calculates desired forward lean (forward lean when v_ref > xdot)
                pitch_corr_target = clamp(
                    VEL_KP * vel_err + VEL_KI * vel_integral,
                    -PITCH_CORR_MAX,
                    PITCH_CORR_MAX
                )
                desired_pitch_target = PITCH_OFFSET - pitch_corr_target

                # Slew rate limiter: ensures pitch target updates slowly and smoothly
                max_slew_step = MAX_PITCH_SLEW * dt
                pitch_diff = desired_pitch_target - pitch_target
                pitch_target += clamp(pitch_diff, -max_slew_step, max_slew_step)
                theta_ref = pitch_target

                # Position reference handling: don't fight velocity during cruising
                if abs(v_ref) > 0.01 or abs(xdot) > 0.03:
                    x_ref_active = x
                    x_ref = x
                else:
                    x_ref = x_ref_active
                ...
                # Active position hold only when fully stopped
                pos_gain = Kx if (abs(v_ref) < 0.01 and abs(xdot) < 0.03) else 0.0
                vel_damping = 0.04  # Gentle damping for fast inner loop

                wheel_u_raw = clamp(
                    Kth   * (theta - theta_ref)
                    + Kthd * theta_dot
                    - vel_damping * (xdot - v_ref)
                    - pos_gain * (x - x_ref),
            ...
            if jump_state == STATE_BALANCE:
                ...
                if VELOCITY_PI_ENABLE:
                    if manual_jump_requested and not jump_done and stable_for_jump:
            ...
            if jump_state in [STATE_BALANCE, STATE_SETTLE]:
                # Smooth height transition to desired crouch/stand level
                diff = runtime_crouch_target - current_crouch
                current_crouch += clamp(diff, -CROUCH_TRANSITION_SPEED * dt, CROUCH_TRANSITION_SPEED * dt)

                # Symmetrical crouch offsets: hip bends forward (+), knee bends backward (-)
                hip_crouch_offset = current_crouch * 0.14
                knee_crouch_offset = -current_crouch * 0.26

                hip_target_L = hip_stand_L + hip_crouch_offset
                hip_target_R = hip_stand_R + hip_crouch_offset
                knee_target_L = knee_stand_L + knee_crouch_offset
                knee_target_R = knee_stand_R + knee_crouch_offset
            ...
            if step % print_every == 0:  # During balance: print every 0.50 s; During jump: print every 0.02 s
                if wheel_sat:
                    ...
                    status = "correcting"
                height_percent = int((1.0 - current_crouch) * 100)
                print(
                    f"{data.time:6.2f} "
                    f"{runtime_v_ref:+6.2f} "
                    f"{height_percent:4d}% "
                    f"{math.degrees(pitch):+8.2f} "
                    f"{math.degrees(theta):+8.2f} "
                    f"{math.degrees(theta_dot):+7.1f} "
                    f"{xdot:+7.3f} "
                    f"{wheel_u:+7.3f} "
                    f"{x_curr:+6.2f} "
                    f"{move_dir:>9} "
                    f"{jump_state:>10} "
                    f"Fn={Fn_total:5.1f}N "
                    f"Hip={hip_target_L:+5.2f} "
                    f"Knee={knee_target_L:+5.2f} "
                    f"COMz={com[2]:.3f} "
                    f"{status}",
                    flush=True
                )
