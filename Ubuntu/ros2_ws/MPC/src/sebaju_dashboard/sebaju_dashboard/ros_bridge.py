...
# /sebaju/debug:        [t, theta, theta_dot, s, s_dot, wheel_u, com_z, com_z_vel, Fn_total, min_wheel_gap, loaded]
# /sebaju/wbr_state:    [t, s, s_ref, theta, theta_ref, phi, phi_ref, s_dot, s_dot_ref, z, z_ref, delta_s, F_z,
#                        tau_l, tau_r, l, tau_hip_l, tau_knee_l]
# /sebaju/estimation_error: [ex, ey, ez, evx, evy, evz]
STREAMS = {
    'debug': 11,
    'wbr': 18,
    'est': 6,
...
def remove_stale_segments():
    ...
    for entry in entries:
        match = re.match(r'sebaju_dash_(\d+)_', entry)
        if match is None:
            continue
        try:
...
class SharedState:
    ...
    def __init__(self, prefix=None, create=True):
        self.prefix = prefix or f'sebaju_dash_{mp.current_process().pid}_{int(time.time())}'
        self.create = create
...
def run_ros_process(prefix, commands, events):
    ...
    rclpy.init()
    node = Node('sebaju_dashboard', parameter_overrides=[Parameter('use_sim_time', value=True)])
    st = shared.status
    ...
    for topic, name in (('/sebaju/debug', 'debug'), ('/sebaju/wbr_state', 'wbr'),
                        ('/sebaju/estimation_error', 'est'), ('/wheel_effort_controller/commands', 'wheel_cmd')):
    ...
    node.create_subscription(Contacts, '/sebaju/left_wheel_contact', lambda m: on_contact('left', m), fast)
    node.create_subscription(Contacts, '/sebaju/right_wheel_contact', lambda m: on_contact('right', m), fast)
    node.create_subscription(Odometry, '/sebaju/odom', on_odom, fast)
    node.create_subscription(String, '/sebaju/jump_state', on_phase, 10)
    node.create_subscription(String, '/robot_description', on_description, latched)

    running = [True]
    parent = os.getppid()
    cmd_vel_pub = node.create_publisher(Twist, '/sebaju/cmd_vel', 10)
    cmd_height_pub = node.create_publisher(Float64, '/sebaju/cmd_height', 10)
    cmd_jump_pub = node.create_publisher(Empty, '/sebaju/cmd_jump', 10)
    cmd_push_pub = node.create_publisher(Float64, '/sebaju/cmd_push', 10)
