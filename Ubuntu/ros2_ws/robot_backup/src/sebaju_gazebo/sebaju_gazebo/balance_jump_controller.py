...
class BalanceJumpController(Node):
    ...
    def _publish_wheels(self, wheel_u):
        torque = WHEEL_GEAR * clamp(wheel_u, -1.0, 1.0)
        self.wheel_pub.publish(Float64MultiArray(data=[torque, torque]))
    ...
    def _control_step(self, t_abs, dt, q, qd):
        ...
        wheel_u_raw = clamp(
            k_th * (theta - theta_ref) + k_thd * theta_dot - k_xd * (xdot - xdot_ref) - k_x * (x - x_ref),
