"""
SeBaJu test bench logger: subscribes to every topic published by
...
class TestBenchLogger(Node):
    ...
    def __init__(self):
        super().__init__('sebaju_test_bench_logger')
        ...
        filename = f"sebaju_test_bench_{datetime.now():%Y%m%d_%H%M%S}.csv"
        ...
        self.create_subscription(Float64MultiArray, '/sebaju/debug', self._debug_cb, 10)
        self.create_subscription(String, '/sebaju/jump_state', self._jump_state_cb, 10)
