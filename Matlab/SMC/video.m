...
% 1. Recupero parametri fisici (devono coincidere con init_scooter.m)
...
% 2. Estrazione dati dal blocco "To Workspace" (salvato come sim_data)
% Nota: nelle nuove versioni di MATLAB/Simulink i dati sono nel log 'out'
t = out.tout; 
data = out.sim_data; 

% La matrice 'data' ha 4 colonne corrispondenti al nostro Mux: [s, q1, q2, q3]
s_act = data(:, 1);
q1 = data(:, 2);
q2 = data(:, 3);
q3 = data(:, 4);
...
% Creiamo la matrice Q attesa dalla funzione di animazione
Q = [qw, q1, q2, q3];
...
% =========================================================================
% FUNZIONE DI ANIMAZIONE VERTICALE
% =========================================================================
function animate_robot(t, Q, params, filename)
    ...
    % Torso disegnato come un rettangolo spesso per renderlo più visibile
    ...
    for i = 1:length(t_interp)
        % --- CINEMATICA VERTICALE PIANO SAGITTALE ---
        % Ruota
        O0_x = Rw * qw(i);
        O0_y = Rw;
        
        % Ginocchio (Piegato all'indietro o in avanti in base a q1)
       O1_x = O0_x - l1 * sin(q1(i));
O1_y = O0_y + l1 * cos(q1(i));

O2_x = O1_x - l2 * sin(q1(i) + q2(i));   % stesso segno di O1
O2_y = O1_y + l2 * cos(q1(i) + q2(i));
        ...
        % Il raggio ruota per visualizzare il rotolamento
        ...
        % Muovi il terreno in modo che copra sempre la vista
